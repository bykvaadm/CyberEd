...
3. Генерируем сертификаты (где угодно, где есть openssl)
...
4. Подготовливаем сертификаты для загрузки на сервер.
...
5. Подготовливаем конфиг nginx для загрузки на сервер nginx-1. Для этого прописываем указанный ниже конфиг в
...
6. Раскомментируем строки ssl_client_certificate и ssl_verify_client в кинфиге, который мы принесли в п.6 и запускаем
...
7. Ограничиваем доступ только для конкретных сертификатов.
...
8. Опционально попробуйте директивы allow и deny для того чтобы разрешить подключаться только с определённых адресов.
